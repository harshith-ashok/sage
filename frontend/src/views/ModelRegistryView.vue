...
<template>
  <div class="mx-auto max-w-3xl px-8 py-8">
    <div class="mb-5 flex items-center justify-between">
      <div>
        <h1 class="font-display text-lg font-semibold text-text">
          Model Registry
        </h1>
        <p class="mt-1 text-[12px] text-dim">
          Switching here edits
          <code class="font-mono text-dim-2">models.yaml</code> live — no
          restart, no code touched.
        </p>
      </div>
      <Button
        variant="ghost"
        :disabled="store.loading"
        @click="store.fetchRegistry()"
        >Refresh</Button
      >
    </div>
    ...
    <p
      v-if="store.error"
      class="mb-4 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-[12.5px] text-danger"
    >
      {{ store.error }}
    </p>
      ...
      <div class="flex flex-col gap-3">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10.5px] text-dim-2">
            — polled every 5s; local candidates only fall back under "high" load
          </span>
        </div>

        <p v-if="store.loadError" class="text-[12px] text-danger">
          {{ store.loadError }}
        </p>

        <div v-if="store.load && store.load.loaded_models.length > 0">
          <div class="mb-1 text-[10.5px] uppercase tracking-wide text-dim-2">
            Currently loaded (Ollama)
          </div>
        ...
        <div v-if="store.load && store.load.fallback_log.length > 0">
          ...
          <div class="flex flex-col gap-1.5">
            <div
              ...
              "
            >
              <span class="font-mono text-dim-2">{{
    ...
    <div class="flex flex-col gap-4">
      <Card
        v-for="(cfg, taskType) in store.registry"
        :key="taskType"
      ...
      >
        <div class="flex flex-col gap-2">
          ...
          >
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <span class="truncate font-mono text-[12px] text-text">{{
                  candidate.model_id
                }}</span>
                <StatusPill
                  :status="candidate.location === 'local' ? 'ok' : 'warn'"
                >
                  {{ candidate.location }}
                </StatusPill>
                <StatusPill
                  :status="candidate.locally_available ? 'ok' : 'neutral'"
                >
                  {{ candidate.locally_available ? "pulled" : "not pulled" }}
                </StatusPill>
                <StatusPill v-if="key === cfg.active" status="ok"
                  >active</StatusPill
                >
              </div>
      ...
      <p
        v-if="
          !store.loading &&
          Object.keys(store.registry).length === 0 &&
          !store.error
        "
        class="text-[12.5px] text-dim"
      >
        No task types found.
      </p>
    </div>
